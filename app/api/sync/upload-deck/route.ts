...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isDev) {
      console.info(
        `[sync upload-deck] start user=${session.user.userId} libraryId=${libraryId} name=${JSON.stringify(
          name
        )} filename=${JSON.stringify(originalFilename)} size=${size}`
      );
    }

    const bucket = getBucket(db);

    const uploadStream = bucket.openUploadStream(`${session.user.userId}/${libraryId}`, {
      metadata: {
    ...
    if (isDev) console.info("[sync upload-deck] gridfs upload complete");
    ...
    if (isDev) {
      ...
      console.info(`[sync upload-deck] done in ${dt}ms`);
    }
